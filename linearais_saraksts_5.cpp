...
Node* RemoveNodeBeginning(Node *head)
{
  Node* cur;
  cur = head;
  cur = cur->link; // next node

  if (head != nullptr) // nullptr (keyword) is better than NULL or 0. pointer to nothing
  {
       delete head;
  }
...
Node* PrevPrevNode(Node *head, int x)
{
  Node *cur, *prev, *prevprev;
  cur = head;
...
Node* RemoveBefore(Node *head)
{
  ...
  Node *cur, *prev, *prevprev, *afterHead;
  afterHead = head->link;
  cur = FindNode(head, x);
  ...
  {
    cout << "Node exist!\n";
    if(head->data == x) cout << "There is nothing. Can not remove!";
    else if(afterHead->data == x)
    {
      prev = PreviousNode(head, x);
      head = cur;
      delete prev;
...
Node* AfterNextNode(Node *head, int x)
{
  Node *cur, *next, *afternext;
  cur = head;
...
Node* RemoveAfter(Node *head)
{
  ...
  if(cur)
  {
    cout << "Node exist!\n";
    next = NextNode(head, x);
    if(next == nullptr) cout << "There is nothing. Can not remove!\n";
    ...
    {
      afternext = AfterNextNode(head, x);
      delete next;
      cur->link = afternext;
      cout << "Node has been removed!\n";
    }
